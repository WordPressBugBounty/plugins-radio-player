    ...
    public function check_environment() {
        ...
        if ( version_compare( get_bloginfo( 'version' ), $min_wp, '<' ) ) {
            add_action( 'admin_notices', function () use($min_wp) {
                $message = sprintf( '%1$s requires WordPress version %2$s or higher. Please update your WordPress.', '<strong>Radio Player</strong>', '<strong>' . esc_html( $min_wp ) . '</strong>' );
            ...
            add_action( 'after_plugin_row_radio-player/plugin.php', function () use($min_wp) {
                $message = sprintf( '%1$s requires WordPress version %2$s or higher. Please update your WordPress.', '<strong>Radio Player</strong>', '<strong>' . esc_html( $min_wp ) . '</strong>' );
                echo '<tr class="plugin-update-tr active"><td class="plugin-update colspanchange" colspan="4"><div class="update-message notice inline notice-error notice-alt"><p>' . wp_kses_post( $message ) . '</p></div></td></tr>';
            } );
        }
    }
